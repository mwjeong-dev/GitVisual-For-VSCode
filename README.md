...
PyCharm 스타일 git 도구를 VS Code로 이식하는 extension. 커밋 패널의 인라인 diff + 부분 스테이징, Changelist, 커밋 로그 그래프, 인라인 Blame, 라인 범위 히스토리를 목표로 합니다.
...
src/
  ...
  diff/diffParser.ts                # unified diff ↔ Hunk[] (스테이징/향후 blame/history 공용)
  scm/
    changelistStore.ts              # Changelist 데이터 모델 + 영속화 + 재조정
    changelistProvider.ts           # 네이티브 SCM 뷰 (changelist별 리소스 그룹)
    staging.ts                      # hunk 단위 stage/unstage 플러밍
    commitService.ts                # 전체 인덱스 커밋 / changelist 격리 커밋
  commitPanel/
...
- VS Code는 커스텀 SCM 뷰를 기본 Source Control 뷰에 통합할 방법이 없어, "Git Tools Changelists"가 별도 섹션으로 나란히 표시됨
